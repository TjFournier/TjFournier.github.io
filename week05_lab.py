...
def greekify(text):
    ...
    for element in text:
        if element in greek_alphabet:
            result = greek_alphabet.index(element)
            result = int(result)
            conversion = str(latin_alphabet[result])
            accumilator += conversion
        elif element in latin_alphabet:
            result = latin_alphabet.index(element)
            result = int(result)
            conversion = str(greek_alphabet[result])
            accumilator += conversion
        else:
            accumilator += element
    return accumilator
...
def grapheme_equality(x, y):
    ...
    ch_x = unicodedata.normalize('NFD', x) 
    ch_y = unicodedata.normalize('NFD', y)
